package defpackage;

import java.util.Set;

/* renamed from: gf1  reason: default package and case insensitive filesystem */
/* loaded from: classes8.dex */
public final class C26688gf1 extends AbstractC13793Vtm {
    public JLj f;
    public String g;
    public String h;
    public EnumC47593uFg i;
    public Long j;
    public Boolean k;
    public Double l;

    public C26688gf1() {
        super("BITMOJI_QUICK_REPLY_STICKER_PREVIEW", EnumC45985tCg.BUSINESS, 1.0d, 1.0d);
    }

    @Override // defpackage.InterfaceC42467qug
    public final int b() {
        return 309;
    }

    @Override // defpackage.InterfaceC42467qug
    public final void c(C38303oC7 c38303oC7, Set set) {
        byte[] bArr = new byte[1];
        AbstractC39604p2m.J0(c38303oC7, 2, bArr, this.k, set);
        AbstractC39604p2m.M0(c38303oC7, 3, bArr, this.j, set);
        AbstractC39604p2m.O0(c38303oC7, 5, bArr, this.g, set);
        AbstractC39604p2m.O0(c38303oC7, 6, bArr, this.h, set);
        AbstractC39604p2m.K0(c38303oC7, 7, bArr, this.l, set);
        AbstractC39604p2m.L0(c38303oC7, 8, bArr, this.f, set);
        AbstractC39604p2m.L0(c38303oC7, 9, bArr, this.i, set);
        c38303oC7.j(bArr);
    }
}
