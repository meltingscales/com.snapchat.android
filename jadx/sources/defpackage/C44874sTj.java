package defpackage;

import java.util.Map;
import java.util.Set;

/* renamed from: sTj  reason: default package and case insensitive filesystem */
/* loaded from: classes8.dex */
public final class C44874sTj extends AbstractC29625iZj {
    public String k;
    public EnumC46406tTj l;
    public Double m;

    public C44874sTj() {
        super("SPECTACLES_HERMOSA_PROXY_STARTED_FAILURE", EnumC45985tCg.BUSINESS, 1.0d, 1.0d);
    }

    @Override // defpackage.InterfaceC42467qug
    public final int b() {
        return 2878;
    }

    @Override // defpackage.InterfaceC42467qug
    public final void c(C38303oC7 c38303oC7, Set set) {
        byte[] bArr = new byte[1];
        AbstractC39604p2m.O0(c38303oC7, 2, bArr, this.f, set);
        AbstractC39604p2m.K0(c38303oC7, 3, bArr, this.m, set);
        AbstractC39604p2m.L0(c38303oC7, 4, bArr, this.l, set);
        AbstractC39604p2m.O0(c38303oC7, 5, bArr, this.g, set);
        AbstractC39604p2m.L0(c38303oC7, 6, bArr, this.i, set);
        AbstractC39604p2m.O0(c38303oC7, 7, bArr, this.h, set);
        AbstractC39604p2m.O0(c38303oC7, 8, bArr, this.k, set);
        AbstractC39604p2m.O0(c38303oC7, 9, bArr, this.j, set);
        c38303oC7.j(bArr);
    }

    @Override // defpackage.AbstractC29625iZj, defpackage.AbstractC13793Vtm, defpackage.AbstractC55051z78, defpackage.InterfaceC33853lIc
    public final int d(Map map) {
        EnumC46406tTj enumC46406tTj;
        int d = super.d(map);
        Double d2 = (Double) map.get("failed_setup_time_in_sec");
        this.m = d2;
        if (d2 != null) {
            d++;
        }
        if (map.containsKey("failure_reason")) {
            Object obj = map.get("failure_reason");
            if (obj instanceof String) {
                enumC46406tTj = EnumC46406tTj.valueOf((String) obj);
            } else {
                enumC46406tTj = (EnumC46406tTj) obj;
            }
            this.l = enumC46406tTj;
            d++;
        }
        String str = (String) map.get("proxy_session_id");
        this.k = str;
        if (str != null) {
            return d + 1;
        }
        return d;
    }
}
